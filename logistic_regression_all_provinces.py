...
for file_paths in [file_paths3]:
    for file_path in file_paths:
            ...
            if province != "Canada":
                ...
                model_logistic.fit(X_train_1, Y_train_1)
                y_pred_logistic = model_logistic.predict(X_test_1)
                accuracy_logistic = accuracy_score(Y_test_1, y_pred_logistic)

                # Save accuracy to dictionary
                accuracy_dict[f"{file_name}: {target}"] = accuracy_logistic

                print(f"{file_name}: {target} Accuracy = {accuracy_logistic:.2f}")
...
accuracy_df.to_csv('D:\\winter2023\\CSCI6405\\project\\logictic_regression\\all_province_overall_'+features+'_'+target+'_accuracy.csv')
...
accuracy_df = pd.DataFrame.from_dict(accuracy_dict, orient='index', columns=['Accuracy'])
accuracy_df.index.name = 'Index'
accuracy_df.to_csv('D:\\winter2023\\CSCI6405\\project\\logictic_regression\\all_province_detailed_'+features+'_'+target+'_accuracy.csv')
