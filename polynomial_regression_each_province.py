...
#             test_data1 = df.loc[df['province'] == province].loc[df['year'] == "2021"]
#             train_data1 = df.loc[df['province'] == province].loc[df['year'] != "2021"]
#
...
#                     mse_dict[f"{file_name}: {province}_{column_name}"] = mse_poly
#
#                     print(f"{file_name}: {province}_{column_name} MSE = {mse_poly:.2f}")
#
...
# mse_df.to_csv('D:\\winter2023\\CSCI6405\\project\\linear_regression_each_province\\each_province_overall_economics_2021.csv')
...
for file_paths in [file_paths2]:
    for file_path in file_paths:
        ...
        for province in df['province'].unique():
            ...
            file_name = file_path.split("\\")[-1].split(".")[0]
            test_data1 = df.loc[df['province'] == province].loc[df['year'] == year]
            train_data1 = df.loc[df['province'] == province].loc[df['year'] != year]
...
mse_df = pd.DataFrame.from_dict(mse_dict, orient='index', columns=['MSE'])
mse_df.index.name = 'Index'
mse_df.to_csv('D:\\winter2023\\CSCI6405\\project\\linear_regression_each_province\\each_province_detailed_'+target+'_'+year+'.csv')